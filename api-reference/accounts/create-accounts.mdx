...
<AccordionGroup>
  ...
  <Accordion title="Option 2: Using externalCompanyId (External Reference)" icon="link">
    Use this when you only know the **native external ID** stored on the company record.
    ...
    - `externalCompanyId`: The external ID stored on `companies.external_company_id`.
    - The API resolves this directly from the companies table.
...
<ParamField body="providerKey" type="string">
  Accepted for backward compatibility only. It is not used for account or company lookup.
</ParamField>
...
<ParamField body="accounts" type="array" required>
  ...
  <Expandable title="Account Object">
    ...
    <ParamField body="externalCompanyId" type="string">
      **External company ID** stored on `companies.external_company_id`. Mutually exclusive with `companyId`.
    </ParamField>
    
    <ParamField body="externalAccountId" type="string">
      **External Account ID** stored on `accounts.external_id`. Your internal unique ID for this account from your integration source (e.g. `ACC_123`).
      *Required.*
    </ParamField>
    ...
    <Info>
      **Upsert Logic:**
      ...
      2. **Company Link**: Must resolve `companyId` (internal UUID) OR `externalCompanyId` (`companies.external_company_id`) to an existing parent company.
    </Info>
