...
Entitlements define usage limits and quotas for products within plans and subscriptions. **Entitlements are completely optional**:

#### When to Use Entitlements
- **With entitlements**: Track usage limits, quotas, or metered access
  - Example: 10,000 image analyses per month
...
- **`usage_key`**: The metric being limited (e.g., "image_analysis_count")
- **`quantity`**: The numeric limit (optional, null = unlimited)
- **`validity_interval`**: Time unit (`month`, `year`, `one_time`)
- **`validity_interval_count`**: Number of intervals

#### Entitlement Hierarchy
Entitlements can be defined at multiple levels, with overrides flowing down:
1. **Product-level** (`productEntitlementsJson` in Plans): Default limits for all subscriptions
2. **Subscription-level** (`entitlementsOverrideJson` in Subscriptions): Custom limits for specific customers

**Example**: A plan might include 10,000 images/month by default, but a subscription can override this to 50,000 images/month for a specific enterprise customer.
