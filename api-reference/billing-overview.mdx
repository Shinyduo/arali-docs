...
### Invoices
Invoices represent billable statements for a company/account/subscription:
- Create invoices in batch for sync or generation flows
- Track invoice lifecycle (`draft` → `open` → `paid`, or `void`/`uncollectible`)
- Store provider identifiers (e.g., Stripe invoice IDs) and PDF URLs
- Query invoices with filters for collections and reporting
...
Contract values (ARR) are automatically converted to minor currency units for precision:
...
- **`contractValue`** = ARR (Annual Recurring Revenue). It is stored as-is (in cents) as the subscription's ARR.
- **MRR** is auto-calculated as `ARR / 12` and stored in `mrrCents`.
- When a subscription is created or updated, the company's total ARR is recalculated by summing all active subscriptions.
...
# List open invoices for a company
GET /api/v1/invoices?externalCompanyId=acme-corp&status=open
...
<CardGroup cols={2}>
  ...
  <Card title="Invoices API" icon="receipt" href="/api-reference/invoices/create-invoices">
