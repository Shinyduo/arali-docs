...
The POST endpoint creates or updates subscriptions. When `externalSubscriptionId` is provided and matches an existing subscription, the record is **updated** instead of creating a new one (upsert). This eliminates the need to use the PUT endpoint for syncing from external billing providers.

<Note>
  **Upsert behavior**: If `externalSubscriptionId` is provided, the API will look for an existing subscription with that external ID. If found, it updates all fields. If not found, it creates a new subscription. When no `externalSubscriptionId` is provided, a new subscription is always created.
</Note>
...
<ParamField body="subscriptions" type="array" required>
  ...
  <Expandable title="Subscription Object">
    ...
    <ParamField body="contractValue" type="number">
      Annual Recurring Revenue (ARR) in major currency units (automatically converted to minor units). MRR is auto-calculated as ARR / 12.
    </ParamField>
    ...
    <ParamField body="externalSubscriptionId" type="string">
      External subscription ID from billing provider (e.g., Stripe). **When provided, enables upsert**: if a subscription with this external ID already exists, it will be updated instead of creating a new one.
    </ParamField>
...
### Workflow: Sync from Stripe (Upsert by External ID)
```bash
# Step 5: Sync subscription from Stripe — creates on first call, updates on subsequent calls
POST /api/v1/subscriptions
{
  "subscriptions": [{
    "externalCompanyId": "new-customer",
    ...
    "externalSubscriptionId": "sub_stripe_12345",
    "currentPeriodEndAt": "2027-01-22T00:00:00Z",
    "metadata": { "source": "stripe_webhook" }
  }]
}
# ✅ First call: Creates new subscription
# ✅ Second call: Updates existing subscription (matched by externalSubscriptionId)
```
...
```bash
# Option A: Upsert via external ID (recommended for integrations)
POST /api/v1/subscriptions
{
  "subscriptions": [{
    "externalCompanyId": "new-customer",
...
# ✅ Success: Updates existing subscription matched by externalSubscriptionId

# Option B: Update by internal ID (use PUT)
...
- **`contractValue` represents ARR** (Annual Recurring Revenue) and is automatically converted to minor currency units (cents for USD, no conversion for JPY)
  - Example: USD 99.99 → stored as 9999 cents
  - **MRR** is auto-calculated as `ARR / 12` and stored in `mrrCents`
- **Upsert via `externalSubscriptionId`**: When provided, the API checks for an existing subscription with that external ID. If found, all fields are updated. If not, a new subscription is created. This is ideal for syncing from Stripe, Chargebee, etc.
